...
def estimate_SEPIN_torch_full(z_mean, z_stddev, device, x_weights=None,
...
def estimate_JEMMIG_torch(z_mean, z_stddev, y, x_weights=None, num_samples=10000,
                         seed=None, batch_size=10, eps=1e-8, gpu=0, device=None):
    """
    ...
    H_yk = torch.asarray(H_yk).to(device)
    ...
    H_zi = torch.asarray(np.zeros(z_dim, dtype=np.float32)).to(device)
    # (z_dim, y_dim)
    H_zi_yk = torch.asarray(np.zeros([z_dim, y_dim], dtype=np.float32)).to(device)
    ...
    while count < num_samples:
        ...
        z_batch = torch.unsqueeze(z_samples[count: count + b], axis=0)
        #print('q_zi_cond_x_batch',q_zi_cond_x_batch.shape)
        # Sum over axis=0 of (S_batch, z_dim) => z_dim
        H_zi_cond_x = H_zi_cond_x - torch.sum(torch.log(torch.maximum(q_zi_cond_x_batch, eps[count: count + b, 0, :])), axis=0)
        # --------------------------------- #

        # Computing H_zi
        # --------------------------------- #
        # (1, S_batch, z_dim) and (N, S_batch, z_dim) => (N, S_batch, z_dim)
        q_zi_cond_x_all_batch = normal_density_torch(z_batch, z_mean, z_stddev, eps=eps)
        ...
        # (N, 1) == (1, S_batch) => (N, S_batch)
            q_yk_cond_x_all_batch = np.equal(torch.unsqueeze(y[:, k], axis=-1).detach().cpu().numpy(), torch.unsqueeze(y[rand_ids[count: count + b], k], axis=0).detach().cpu().numpy())

            # print("q_yk_cond_x_all_batch[:10, :10]: {}".format(q_yk_cond_x_all_batch[:10, :10]))

            # (N, S_batch, 1)
            q_yk_cond_x_all_batch = torch.unsqueeze(torch.asarray(q_yk_cond_x_all_batch, dtype=torch.float32).to(device), axis=-1)

            # (N, S_batch, z_dim) * (N, S_batch, 1) then sum over axis=0 => (S_batch, z_dim)
            q_zi_yk_batch = torch.sum(torch.unsqueeze(torch.unsqueeze(x_weights, axis=-1), axis=-1)*q_zi_cond_x_all_batch * q_yk_cond_x_all_batch, axis=0)

            # Sum (S_batch, z_dim) over S_batch => (z_dim, )
            H_zi_yk[:, k] += -torch.sum(torch.log(torch.maximum(q_zi_yk_batch, eps[count: count + b, 0, :])), axis=0)
    ...
    H_zi_cond_x = H_zi_cond_x.cpu().detach().numpy()
    H_zi = H_zi.cpu().detach().numpy()
    H_zi_yk = H_zi_yk.cpu().detach().numpy()
    H_yk = H_yk.cpu().detach().numpy()
    # (z_dim, )
    H_zi_cond_x = H_zi_cond_x / (1.0 * num_samples)
    H_zi = H_zi / (1.0 * num_samples)
    H_zi_yk = H_zi_yk / (1.0 * num_samples)

    print("H_yk: {}".format(H_yk.shape))
    print("\nH_zi: {}".format(H_zi.shape))
    print("\nH_zi_yk:\n{}".format(H_zi_yk.shape))
    ...
    JEMMIG = np.mean(JEMMIG_yk, axis=0)
...
def main():
    ...
    z_mean = np.random.random((13600, 100))*0.3
   
    z_std = np.random.random((13600, 100))
    y = np.random.random((13600, 10))

    t_z_mean = torch.asarray(z_mean).to(device)
    t_z_std = torch.asarray(z_std).to(device)
    t_y = torch.asarray(y).to(device)
    ...
    results_jemmig = estimate_JEMMIG_torch(t_z_mean, t_z_std, y, num_samples=10000, batch_size=250, device=device)

    print(results_jemmig)
