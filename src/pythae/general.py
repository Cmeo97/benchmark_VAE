...
def normal_density_cupy(x, mean, stddev, from_axis=None, eps=1e-8, gpu=0):
    ...
    with cp.cuda.Device(gpu):
        ...
        density = cp.exp(-cp.square(cp.sum((x - mean), axis=2)) / (2 * cp.sum(variance, axis=2))) / (cp.sum(stddev, axis=2) * math.sqrt(2 * math.pi))

        if (from_axis is not None) and (from_axis >= 0):
            shape = tuple(density.shape[:from_axis]) + (density.shape[from_axis:][0],)
...
#def normal_density_torch(x, mean, stddev, from_axis=None, eps=1e-8):
#    
#    
#    
#    variance = torch.maximum((stddev) ** 2, eps)
#    stddev = torch.maximum(stddev, eps)
#
#    density = torch.exp(- torch.square(torch.sum((x - mean), axis=2)) / (2 * torch.sum(variance, axis=2))) / (torch.sum(stddev, axis=2) * math.sqrt(2 * math.pi))
#
#    if (from_axis is not None) and (from_axis >= 0):
#        shape = tuple(density.shape[:from_axis]) + (density.shape[from_axis:][0],)
#        density = torch.reshape(density, shape)
#        density = torch.prod(density, axis=from_axis)
#
#    return density    

def normal_density_torch(x, mean, stddev, from_axis=None, eps=1e-8, gpu=0):
 

    variance = torch.maximum(stddev ** 2, eps)
    stddev = torch.maximum(stddev, eps)
    density = torch.exp(-torch.square(x - mean) / (2 * variance)) / (stddev * math.sqrt(2 * math.pi))
    if (from_axis is not None) and (from_axis >= 0):
        shape = tuple(density.shape[:from_axis]) + (torch.prod(density.shape[from_axis:]),)
        density = torch.reshape(density, shape)
        density = torch.prod(density, axis=from_axis)
    return density
...
def lighter_normal_density_torch(x, mean, stddev, from_axis=None, eps=1e-8):
    ...
    for i in range(x.shape[1]):
        for j in range(x.shape[2]):
            density_.append(torch.exp(-torch.square(torch.sum((torch.unsqueeze(x[:, i, j], axis=1) - mean[:, :, j]), axis=2)) / (2 * torch.sum(variance[:, :, j], axis=2))) / (torch.sum(stddev[:, :, j], axis=2) * math.sqrt(2 * math.pi)))
        density__.append(torch.unsqueeze(torch.cat(density_, axis=1), axis=1))
        density_ = []
    density = torch.cat(density__, axis=1)
...
def lighter_normal_density_cupy(x, mean, stddev, from_axis=None, eps=1e-8, gpu=0):
    import cupy as cp
    ...
    with cp.cuda.Device(gpu):
        ...
        for i in range(x.shape[1]):
            for j in range(x.shape[2]):
                density_.append(cp.exp(-cp.square(cp.sum((cp.expand_dims(x[:, i, j], axis=1) - mean[:, :, j]), axis=2)) / (2 * cp.sum(variance[:, :, j], axis=2))) / (cp.sum(stddev[:, :, j], axis=2) * math.sqrt(2 * math.pi)))
            density__.append(cp.expand_dims(cp.concatenate(density_, axis=1), axis=1))
            density_ = []
        density = cp.concatenate(density__, axis=1)
